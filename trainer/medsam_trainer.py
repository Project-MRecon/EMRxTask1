...
class ResnetTrainer(BaseTrainer):
    def __init__(self, plans, data_loader, valid_data_loader=None):
        ...
        self.writer_step = 5
        self.swa_scheduler = SWALR(self.optimizer, anneal_strategy="linear", anneal_epochs=10, swa_lr=0.05)
    ...
    def _train_epoch_end(self, epoch, epoch_loss, batch_idx):
        if self.lr_scheduler is not None:
            if epoch < 6:
                ...
                self.logger.info("last_lr:{}".format(self.lr_scheduler.get_last_lr()))
                self.logger.info("lr:{}".format(self.lr_scheduler.get_lr()))
            else:
                self.swa_scheduler.step()
                self.writer.add_scalar("lr", self.swa_scheduler.get_lr(), epoch)
        epoch_loss /= batch_idx  
        log = {"loss": epoch_loss}
