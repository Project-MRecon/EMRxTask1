...
#%%
    # create training-validation data loaders "/homes/syli/dataset/MultiCoil/test/test.json" args.data_json_path
    with open("/homes/syli/dataset/MultiCoil/test/lax.json") as f:
        data = json.load(f)
    ...
    train_transforms = Compose(
        [
            ...
            CenterSpatialCropd(keys=["kspace_masked_ifft", "reconstruction_rss"], roi_size=[168, 416]),
            ReferenceBasedNormalizeIntensityd(
    ...
    train_ds = CacheDataset(
        data=data["train"], transform=train_transforms, cache_rate=0, num_workers=4
    )
    # for i in train_ds:
    #     print(i["kspace_masked_ifft"].meta["filename_or_obj"])
    #     break
#%%
    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)

    val_ds = CacheDataset(
        data=data["val"], transform=train_transforms, cache_rate=args.cache_rate, num_workers=args.num_workers
    )
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                for val_data in val_loader:
                    ...
                    fname = input.meta["filename_or_obj"]
...
def __main__():
    ...
    parser.add_argument(
        "--data_json_path",
        default="/homes/syli/dataset/MultiCoil/test/test.json",
        type=Path,
    ...
    parser.add_argument("--exp_dir", default="/homes/syli/python/CMRxRecon2024/saved/log", type=Path, help="output directory to save training logs")
    ...
    parser.add_argument(
        "--mask_type", default="equispaced", type=str, help="under-sampling mask type: ['random','equispaced']"
    )
