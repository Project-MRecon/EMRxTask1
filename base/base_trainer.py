...
class BaseTrainer(object):
    ...
    def __init__(self, plans, data_loader, valid_data_loader=None,):
        self.data_loader = data_loader # 训练数据 可以考虑在train函数传入
        self.valid_data_loader = valid_data_loader  # 验证数据 同上
        ...
        self.device = "cuda"
        self.scaler = torch.cuda.amp.GradScaler()  # 设置amp
    ...
    # build model architecture, then print to console
        network_class = getattr(models, self.plans.network['arch'])
    ...
    def _load_metrics(self):
        ...
        for key in metrics_dict:
            one_metric = getattr(models.metric, key+"Metric")
    ...
    def _train_epoch(self, epoch):
        ...
        log = self._train_epoch_end(epoch, epoch_loss, batch_idx)
    ...
    def _update_metrics(self, pred, label):
        for metric in self.plans.Metrics:
            self.metrics[metric](y_pred=pred, y=label)
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        if checkpoint['config']['network']['arch'] != self.plans.network['arch']:
            self.logger.info("Warning: Architecture configuration given in config file is different from that of "
        ...
        if checkpoint['config']['optimizer']['type'] != self.plans.optimizer['type']:
            self.logger.info("Warning: Optimizer type given in config file is different from that of checkpoint. "
    ...
    def _train_epoch_end(self, epoch, epoch_loss, batch_idx):
        if self.lr_scheduler is not None:
            self.lr_scheduler.step()
            self.writer.add_scalar("lr", self.lr_scheduler.get_last_lr(), epoch)
        epoch_loss /= batch_idx  
        log = {"loss": epoch_loss}
        for key, value in self.metrics.items():
            log[key] = value.aggregate().cpu().numpy()
        return log
